...
export function CaregiverDashboard({ patients }: CaregiverDashboardProps) {
  ...
  const delayedPatients = patients.filter((p) => p.status !== "A tiempo");

  const handleNotifyAll = () => {
    const toNotify = delayedPatients.filter((p) => !notifiedIds.includes(p.id));
    toNotify.forEach((p) => {
      console.log(
        `[SIMULATED SMS] Notificación a cuidador sobre paciente ${p.name}: estado ${p.status} - ${p.lastActivity?.delayStr ?? p.lastActivity?.state ?? ""}`,
      );
    });
    setNotifiedIds((prev) => [...prev, ...toNotify.map((t) => t.id)]);
  };
  ...
  return (
    <div className="flex-1 w-full flex flex-col min-h-screen bg-[#F8FAFC]">
      ...
      <header className="flex items-center justify-between px-8 py-4 bg-white border-b border-slate-200">
        ...
        <LogoutButton className="flex items-center gap-2 text-sm font-semibold text-red-600 bg-transparent hover:bg-red-50 hover:text-red-700 shadow-none border-none px-4 py-2 rounded-lg transition-colors h-auto">
          Cerrar sesión
          <svg
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-8 py-8 flex flex-col gap-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div>
            ...
            <p className="text-[#64748B] text-[15px] mt-1">
              Vista consolidada del estado de cumplimiento de los tratamientos
              de mis pacientes asignados.
            </p>
          </div>

          <div className="flex items-center gap-3">
            {/* Export and patient filter removed as requested */}
          </div>
        </div>

        {/* Avisos: pacientes atrasados */}
        <div className="mt-4">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-sm font-bold text-yellow-800">
                    {delayedPatients.length} paciente
                    {delayedPatients.length === 1 ? "" : "s"} con tomas
                    atrasadas
                  </p>
                  <p className="text-sm text-yellow-700">
                    Revisa los pacientes y confirma recordatorios.
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <p className="text-sm text-yellow-800">
                  Revisa los pacientes atrasados desde la lista.
                </p>
              </div>
            </div>
          ) : (
            <div className="bg-green-50 border border-green-100 rounded-2xl p-4 flex items-center gap-4 shadow-sm">
              <div className="w-10 h-10 rounded-full bg-green-100 flex items-center justify-center text-green-800 font-bold">
                ✓
              </div>
              <div>
                <p className="text-sm font-bold text-green-800">
                  Ningún paciente atrasado
                </p>
                <p className="text-sm text-green-700">
                  Todos los pacientes están a tiempo con sus tomas.
                </p>
              </div>
            </div>
          )}
        ...
        {patients.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {patients.map((patient) => {
              ...
              return (
                <div
                  key={patient.id}
                  className={`bg-white rounded-[24px] border ${borderClass} flex flex-col p-6 transition-all hover:-translate-y-1 hover:shadow-lg`}
                >
                  {/* Top Row: Avatar, Name, Status */}
                  <div className="flex items-start justify-between mb-8">
                    <div className="flex items-center gap-4">
                      <div
                        className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-lg ${avatarBgClass} ${avatarTextClass}`}
                      >
                        {patient.initials}
                      </div>
                      <div>
                    ...
                    <div
                      className={`px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1.5 ${badgeBgClass} ${badgeTextClass}`}
                    >
                      <div
                        className={`w-1.5 h-1.5 rounded-full ${isGreen ? "bg-[#059669]" : isOrange ? "bg-[#D97706]" : "bg-[#DC2626]"}`}
                      ></div>
                      {patient.status}
                    </div>
                  </div>
                      ...
                      {patient.lastActivity ? (
                        <div className="flex items-start gap-2.5">
                          {patient.lastActivity.state === "Tomó a tiempo" ? (
                            <CheckCircle2
                              size={18}
                              className={`mt-0.5 ${activityIconClass}`}
                            />
                          ...
                          <div className="flex flex-col gap-0.5">
                            <span
                              className={`text-sm font-bold ${activityIconClass}`}
                            >
                              {patient.lastActivity.state}
                            </span>
                            <span className="text-[13px] text-[#64748B] font-medium">
                              {patient.lastActivity.timeStr}
                            </span>
                            <span className="text-[13px] text-[#94A3B8]">
                              {patient.lastActivity.medication}
                            </span>
                            {patient.lastActivity.delayStr && (
                              <span
                                className={`text-[12px] font-bold mt-1 ${activityIconClass}`}
                              >
                                {patient.lastActivity.delayStr}
                              </span>
                            )}
                  ...
                  <div className="mt-auto pt-2 flex justify-center">
                    <Link
                      href={`/protected/cuidador/patients/${patient.id}`}
          ...
          <div className="flex-1 flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-slate-200">
            ...
            <p className="text-slate-500 max-w-md text-center">
              Actualmente no tienes pacientes asignados. Pide a un administrador
              que asigne pacientes a tu perfil usando la tabla de
              user_relations.
            </p>
          </div>
        )}
        ...
        <div className="mt-4 bg-[#EFF6FF] rounded-2xl p-4 flex items-center gap-3">
          ...
          <p className="text-sm text-[#1E40AF] font-medium">
            La información se actualiza automáticamente conforme los pacientes
            registran sus tomas.
          </p>
        </div>
      </main>
