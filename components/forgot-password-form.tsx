...
export function ForgotPasswordForm() {
  ...
  // Dentro de tu ForgotPasswordForm
const handleCheckEmail = async (e: React.FormEvent) => {
  ...
  try {
    // Usamos el RPC que ya tiene los permisos definidos
    const { data: userQuestions, error: rpcError } = await supabase
      .rpc('get_user_questions_by_email', { p_email: email });

    if (rpcError) throw rpcError;

    // Si no devuelve filas, el correo no existe en auth.users
    ...
    // Evitamos el renderizado de objetos vacíos
        ...
        <CardContent>
          {step === 1 && (
            <form onSubmit={handleCheckEmail} className="space-y-4">
              ...
              <Button type="submit" className="w-full bg-[#A8D8FF] text-black" disabled={isLoading}>
                {isLoading ? <Loader2 className="animate-spin" /> : "Verificar Email"}
              </Button>
            </form>
          )}
          ...
          {step === 2 && (
            <form onSubmit={handleVerifyAndSend} className="space-y-4">
              ...
              {error && <p className="text-xs text-red-500 bg-red-50 p-2 rounded">{error}</p>}
              <Button type="submit" className="w-full bg-[#A8D8FF] text-black" disabled={isLoading}>
                {isLoading ? <Loader2 className="animate-spin" /> : "Enviar enlace de recuperación"}
              </Button>
            </form>
          )}
