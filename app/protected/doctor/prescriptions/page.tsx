...
import Link from "next/link"; // <-- IMPORTANTE: Añadimos el componente Link
...
async function DoctorPatientContent() {
  ...
  (patients ?? []).forEach((patient: any) => {
    ...
    rawLogs.forEach((log: any) => {
      if (log.status === "cumplido" || log.status === "atrasado" || log.status === "cumplido") {
        onTimeCount++;
      }
      totalCount++;
    });
  ...
  return (
    <div className="flex-1 w-full flex flex-col min-h-screen bg-[#BDE5FF] p-8">
      <header className="flex items-center justify-between bg-transparent mb-8 px-4">
        ...
        {/* Agrupamos los botones en un contenedor con alineación a la derecha */}
        <div className="text-right">
          {/* NUEVO BOTÓN: Redirige al panel de analíticas de adherencia */}
          <Link
